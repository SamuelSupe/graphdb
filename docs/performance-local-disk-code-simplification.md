...
唯一未分类事件出现在 `03:36:30.488Z`，恰好对应 `03:06:30.488Z` 开始的 30 分钟全局上下文截止时间。补齐与其他采样一致的分类后，同一份未修改原始日志通过报告校验；关闭窗口外超时和非超时错误仍由回归测试保护。本次只修改报告工具和版本元数据，数据库实现与候选 `.3` 相同。使用新标签 `v1.3.4-local.4` 重新执行完整发行门禁，成功后才发布；实际运行日志随发行包提供。
