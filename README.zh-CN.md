...
| 领域 | 改进 |
| --- | --- |
| Commit tail 与图加载 | commit-tail replay 支持有界并发并继续按版本顺序应用；head 前进时 compact 保留新 tail，避免与维护过程冲突。 |
| Reader 内存与新鲜度 | entity page 的 Arrow 字符串及时释放；成功写入后通过 write-through reader cache 发布更新。 |
| 查询分配与边界 | indexed range/aggregate 路径使用有界 top-K 收集结果；fuzzy 避免逐实体过滤器和字符串分配。 |
| 固定环境证据 | 相对证据，不是生产 SLO：storage tail-31 `-38.37%`、compact `-24.99%`、in-use heap `-43.75%`，120s 8W/8R soak 为 `PASS`；native query c64 range QPS `70.97→777.09`、p95 `1028.15→49.28 ms`；fuzzy QPS `1251.31→2568.26`、p95 `48.955→12.305 ms`。 |
| 服务级边界 | HTTP、stream、saved-query、freshness 和 mixed 性能矩阵仍为 `UNKNOWN`；以上进程内与存储数字不代表这些路径已通过。 |
...
这是下文记录的 v1.2.0 验收合同，不能把它表述成额外的 v1.2.3 证据。
